...
export default function Dashboard({
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 w-full lg:w-[420px] shrink-0">
            <div className="rounded-3xl border border-slate-800/70 bg-slate-950/80 p-4 shadow-xl shadow-slate-950/15">
              ...
              <select
                ...
                className="mt-3 w-full cursor-pointer rounded-2xl border border-slate-800/75 bg-slate-950/90 px-4 py-3 text-sm text-slate-100 outline-none transition hover:border-cyan-400/80 focus:border-cyan-300/90 focus:ring-2 focus:ring-cyan-400/20"
              >
      ...
      {/* 2. Kunci Wawasan (Horizontal Row) */}
      ...
      <section className="text-left rounded-3xl border border-slate-800/70 bg-slate-950/85 p-5 shadow-xl shadow-slate-950/10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            onClick={() => setCurrentTab('OT History')}
            className="inline-flex items-center justify-center rounded-3xl bg-cyan-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-cyan-400"
          >
            Lihat Semua History
          </button>
        </div>
        <div className="mt-5 grid gap-4">
          {stats.recentActivities.length > 0 ? (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-300">
                      {activity.namaKaryawan} - {activity.jenisLembur}
                    </p>
                  </div>
                  <span className="rounded-2xl bg-slate-800/75 px-3 py-1 text-xs uppercase tracking-[0.3em] text-slate-300">
                    {activity.statusDocument}
                  </span>
                </div>
            ...
            <div className="rounded-3xl border border-slate-800/75 bg-slate-950/90 p-6 text-center text-slate-400">
              Tidak ada aktivitas lembur untuk periode ini.
            </div>
          )}
        </div>
      </section>
